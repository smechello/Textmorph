...
def get_user_teams(username):
    ...
    """
    df = pd.read_sql_query(query.replace("?", f"'{username}'"), conn)
    conn.close()
    return df
...
def get_team_details(team_id):
    ...
    conn = sqlite3.connect("users.db")
    df = pd.read_sql_query(f"SELECT * FROM teams WHERE id={team_id}", conn)
    conn.close()
...
def get_team_members(team_id):
    ...
    query = """
        ...
        WHERE tm.team_id = ?
    """
    df = pd.read_sql_query(query.replace("?", f"{team_id}"), conn)
    conn.close()
    return df
...
def get_conn():
    """Establishes a connection, prioritizing cloud DB over local."""
    # Connect to PostgreSQL if the secret is available (on Streamlit Cloud)
    if "DB_CONNECTION_STRING" in st.secrets:
        try:
            engine = sqlalchemy.create_engine(st.secrets["DB_CONNECTION_STRING"])
            return engine
        except Exception as e:
...
def get_all_other_users(current_username):
    ...
    conn = sqlite3.connect("users.db")
    df = pd.read_sql_query(f"SELECT username, name, email FROM users WHERE username != '{current_username}'", conn)
    conn.close()
    return df
...
def get_friendship(user1, user2):
    ...
    conn = sqlite3.connect("users.db")
    df = pd.read_sql_query(f"SELECT * FROM friends WHERE user1_username='{u1}' AND user2_username='{u2}'", conn)
    conn.close()
    return df.iloc[0] if not df.empty else None
...
def get_friend_requests(username):
    ...
    conn = sqlite3.connect("users.db")
    query = f"SELECT * FROM friends WHERE (user1_username='{username}' OR user2_username='{username}') AND status='pending' AND action_user_username != '{username}'"
...
def get_friends_list(username):
    ...
    query = f"""
        ...
        FROM friends
        WHERE (user1_username = '{username}' OR user2_username = '{username}') AND status = 'accepted'
    """
    df_friends = pd.read_sql_query(query, conn)
    ...
    friend_usernames = df_friends['friend_username'].tolist()
    query_details = f"SELECT username, name, profile_pic, last_seen FROM users WHERE username IN ({','.join([f'\'{u}\'' for u in friend_usernames])})"
...
def get_direct_messages(user1, user2):
    ...
    query = f"""
        ...
        WHERE (s.sender_username = '{user1}' AND s.recipient_username = '{user2}')
           OR (s.sender_username = '{user2}' AND s.recipient_username = '{user1}')
